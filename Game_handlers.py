...
from lexicon import *
from UserFilter import (BOT_WIN, SET_ATT, CHEMPION, ATTRIBUT_SET_ATTEMPTS_IS_SET,
...
@Game_router.message(SET_ATT(), DATA_IS_DIGIT(), USER_NUMBER())
...
async def bot_win(message: Message):
    ...
    std_out_logger.info(f'******* {users[message.from_user.id]["user_name"]}  имеет статус set_attempts = {users[message.from_user.id]["set_attempts"]}')
    await message.answer(
    ...
    std_out_logger.info(
        f'\n///////////  {users[message.from_user.id]["user_name"]}  имеет статус set_attempts = {users[message.from_user.id]["set_attempts"]}')

@Game_router.message(DATA_IS_DIGIT(),USER_NUMBER())#lambda message: users[message.from_user.id]['user_number'] == 'setting_data' and message.text.isdigit())
